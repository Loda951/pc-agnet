...
- Tool Planner：只消费已准入、query 已冻结的 subquery，负责首轮业务 Tool 选择。
...
- 有依赖关系的 Tool Call 必须等前一轮 Observation 返回后，由 Recovery Planner 在下一轮生成。
...
- 首轮使用 Tool Planner，只绑定 6 个业务 Tool。
...
- 仅当仍有未调用的 routed subquery、可恢复失败、订单候选或明确比较依赖时，才使用同时绑定业务
  Tool 与终止控制 Tool 的 Recovery Planner。
- Router 可为明确单一事实来源的 `tool_planning` subquery 输出受限 `capability`。只有 capability
  属于当前高精度白名单，且 Runtime 的确定性 veto 同意时，Runtime 才直接构造首个 Tool wave 并
  跳过首次 Tool Planner LLM；任何歧义、不一致、复杂比较或不支持的 capability 都回退正常 Planner。
...
schema；Router capability 直达时还可跳过首次 Tool Planner 网络往返。
...
Deterministic pre-route gate
    ...
    -> Router capability 直达首个 Tool wave，或 Tool Planner #1
    -> Tool Wave #1
...
`442 passed, 41 skipped` 且 Ruff 通过。该离线控制流验证不替代真实 LLM 回复质量评测。
