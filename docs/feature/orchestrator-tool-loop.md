# 主编排进度：Request Router + Planner / Answer Loop
...
本阶段已把原先同时承担请求理解、Tool 选择和回答收口的单一 Orchestrator 拆成三个 LLM 角色：

- Request Router：任何业务 Tool 之前完成 query rewrite、上下文融合、Task DAG 拆分与逐项准入。
- Tool Planner：只消费当前依赖已满足的 ready task，负责业务 Tool 选择。
- Answer Synthesizer：消费 Tool Result，直接生成最终中文正文并通过受限控制 Tool 终止；仅在明确
  恢复或依赖调用时切换到 Recovery Planner。
...
业务 Tool Call 不再承担首次 query rewrite；它来自 Tool Planner、Recovery Planner 或通过安全校验
的 Router capability 直达 wave。

编排层仍只通过正式 Tool Contract 使用业务能力。最新 Catalog 与 Knowledge 改动发生在 Tool
内部：前者确定性承接显式数量和销量排序，后者优化检索模式、命中分块和模型缓存；这些变化不把
领域查询规则重新带回 Router 或主 Tool Planner。
...
- `tool_loop.py`：Planner/Observation 消息、wave 约束、恢复准入与终态决策。
- `execution.py`：业务 Tool wave 的执行、复用、事件发送和调用审计。
- `fallback_planner.py`：无 LLM 时仍遵守 route plan 的确定性 Tool 选择。
- `responses.py`：fallback 回答、固定终态模板、blocked notice 与 suggested actions。
- `projections.py`：Tool output 到 products/order/evidence/parsed 的投影。
...
- Tool Planner 首轮看到全部 6 个业务 Tool；Answer Synthesizer 只绑定终止控制 Tool，只有恢复或
  依赖阶段才使用同时绑定业务 Tool 的 Recovery Planner。
- 业务事实必须来自 Tool Result，不能由 LLM 编造。
- `intent` 不参与控制流，只作为兼容的日志和前端 tag 字段。
- Router 必须先生成整轮 `rewritten_query`，再拆成可独立验收的 task；不能先拆分再分别猜测原意。
- 每个 routed task 都有稳定 `sq_n` ID、task canonical query、disposition、`depends_on`、typed
  `input_requirements`、`produces`，必要时还有确定性的 `result_selector`。
...
- Tool Planner 不再输出 query-first Tool 的 `query`；Runtime 根据 `subquery=sq_n` 从 task
...
- Runtime 只调度全部 `depends_on` 已获得 usable 结果的 ready task；无依赖 task 同 wave，依赖 task
  等上游 Observation 后进入下一 wave。具体 capability 通过确定性 veto 时可直接构造任意 ready wave，
  不再限定只能加速首轮。
- Result Normalizer 是无 LLM 的确定性控制层；它只判断结果结构能否作为证据，不生成客服正文。
- 全部 routed subquery 都有 active usable 证据时，Answer Synthesizer 只绑定 `finish_answer`；其他
  Answer Synthesizer 终态也使用 `tool_choice=required`。若模型输出不可解析结果或违规业务 Tool Call，
  Runtime 注入终止反馈并重试一次，Recovery Planner 的合法业务补查不受此限制。
- 每 turn 最多调用 1 次 Request Router；该调用不占后续主编排 LLM 预算。
- 最多执行 2 个 Tool wave，Router 之后的 Planner/Synthesizer/Recovery 调用合计最多 3 次。
...
dispatch_decision
  ...
  |     -> orchestrate                    # Answer Synthesizer；必要时 Recovery Planner
  |
  |-- control action
  |     -> terminal_guard
...
其中只有 `execute_tool_wave -> Tool Results / Observations -> orchestrate` 会回到主编排循环；
...
Tool Planning / Answer Synthesis Layer
    orchestrate (Tool Planner / Answer Synthesizer / Recovery Planner)
...
Durability and Transport Layer
    ...
    compatibility SSE custom events
```
...
Answer Synthesizer 判断全部 Observation 已充分支持回答
    -> grounded_response
...
`direct_response`、`human_handoff`、`out_of_scope`、`unsupported` 和 `security_refusal` 全部由
Request Router 与确定性模板终止。无 `route_plan` 的 legacy Orchestrator binding 已删除；Tool
Planner 只处理准入后的业务子任务。
...
类型：按 state 选择 Tool Planner、Answer Synthesizer 或 Recovery Planner 的 LLM 节点；无 LLM key
时使用遵守 route plan 的确定性 fallback。

生产主路径：

- 每轮先由 Runtime 从 Task DAG 计算 ready task；具体 capability 全部通过确定性 veto 时直接构造
  wave，否则使用只绑定 6 个业务 Tool 的 Tool Planner。
- Tool Result 全部 usable 时使用 finish-only Answer Synthesizer，只绑定 `finish_answer`；存在空结果
  或部分结果但不需要补查时，使用绑定 4 个终止控制 Tool 的 Answer Synthesizer。两者都设置
  `tool_choice=required`，最终中文正文仍由模型直接写入控制 Tool 的 `response`。
- 仅当 ready task 不能安全直达、存在可恢复失败或订单候选时，才使用同时绑定业务 Tool 与终止
  控制 Tool 的 Recovery Planner。
- Router 可为明确单一事实来源的 `tool_planning` subquery 输出受限 `capability`。只有 capability
  属于当前高精度白名单，且 Runtime 的确定性 veto 同意时，Runtime 才直接构造当前 ready wave 并
  跳过本次 Tool Planner LLM；任何歧义、不一致或不支持的 capability 都回退正常 Planner。
- Planner 只接收 ready task 的 `{id, canonical_query, depends_on, input_requirements, produces,
  result_selector}`，不接收原始请求、历史、working memory、长期偏好或 blocked task。
- 为每个 ready task 选择一个必要 Tool；`subquery` 必须复制 `sq_n`，tool query 和依赖参数由 Runtime
  注入，不由 Planner 猜测。
- Tool Observation 返回后，由上述阶段选择决定受限恢复、依赖调用或结构化终止。
...
| Answer Synthesizer | routed subquery、ledger、ToolMessage、Result 解释、客服表达、终止控制 schema | Router 上下文、首轮 Tool 路由、业务 Tool schema、失败恢复规则 |
| Recovery Planner | Answer Synthesizer 所需内容、按需恢复规则、业务 Tool schema | Router 边界分类与 blocked subquery |
...
该分层让首轮 Tool Planner 不携带客服表达内容，也让正常 Answer Synthesizer 不再看到业务 Tool
schema；Router capability 直达时还可跳过对应 ready wave 的 Tool Planner 网络往返。
...
由 `dispatch_route` 处理。Observation 控制动作先经过 `terminal_guard`。`dispatch_decision` 不重新
...
2. 校验 `subquery=sq_n` 是当前 ready task，而不是仍在等待依赖或已被阻断的 task。
3. 由 Runtime 从 task canonical query 派生 `arguments.query`，并分别记录 `canonical_query` 与
   `tool_query`，不要求 Planner 逐字符复制已知文本。
4. 对依赖 task 按 `input_requirements` 绑定上游结果；商品比较把对话中已确认商品和上游选中商品
   合并为 `sku_ids`，不再用单个指代解析结果覆盖一组有效参数。
...
10. 将完整 Tool Result 作为 Answer Synthesizer 或 Recovery Planner 的 Observation。
...
Answer Synthesizer 已经通过终止控制 Tool 返回完整用户正文。运行时根据是否存在成功 Tool Result，
...
Deterministic pre-route gate
    ...
    -> Tool Wave #1
    -> Answer Synthesizer 或 Recovery Planner #2
    -> Tool Wave #2
...
- `orchestrator_call_count <= 3`，统计 Router 之后实际发生的 Tool Planner、Answer Synthesizer 和
  Recovery Planner 调用；Router capability 直达本身不计数。
- `tool_wave_count <= 2`
...
- 下一 wave 只允许两类动作：错误协议允许的同 Tool 恢复；Router Task DAG 中依赖已满足的 ready
  task。`catalog_compare` 要求原始中英文请求明确包含比较意图，且至少两个整数 `sku_ids` 来自
  active `catalog_search` 结果或 working memory 中已确认的当前商品；订单详情只能使用上一轮
...
- Router 输出后、首次 Tool Call 之前 canonical query 已经冻结；首轮 Planner 也不能改写。同一
  ...
  invalid_catalog_plan、empty 和 usage_mapping_unavailable 不允许由 Recovery Planner 改写 query
...
Request Router 先生成 `rewritten_query`，再拆出可独立判断完成状态的 task，并分配稳定的 `sq_n`。
Task 使用 `depends_on` 表示顺序、`input_requirements` 表示待绑定输入、`produces` 表示产物；没有
依赖的 task 自动组成同一 ready wave。Tool Planner 不再拆分请求；每个业务 Tool Call 的
LLM-facing schema 都额外要求
...
该 guard，而是由结构化 route schema、Runtime hard guard 和确定性模板保证。
...
确定性回归覆盖 pre-router fast path、Router rewrite/拆分/准入、Prompt 阶段、Normalizer、ledger、
terminal guard、客服 fallback 和第二轮白名单。Router 专项测试还覆盖模型误放行第三方数据时的
Runtime hard guard、Runtime query 注入、未知 subquery ID 拒绝、blocked subquery 上下文隔离、纯
终态请求零 Router LLM/零业务 Tool、`human_handoff` 穿过完整编译图进入接管模板，以及 mixed
intent 仍调用 Router 并只执行允许部分。
...
`445 passed, 41 skipped` 且 Ruff 通过。该离线控制流验证不替代真实 LLM 回复质量评测。
