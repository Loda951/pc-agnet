...
该节点完成后通过单个 SSE `delta` 事件发送完整正文，不发送 token/chunk 增量。
...
最终正文不会在模型生成过程中发送给用户。`finalize_response` 收到完整正文后，才通过一个完整
`delta` 事件发送，避免部分正文无法撤回，也不再因 `TYPE:` 格式偏差错误进入澄清路径。
...
前端通过单个 delta 展示完整正文，以 `done.response` 作为最终权威状态。
