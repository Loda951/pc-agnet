...
- 紧凑目录使用固定种子的伪随机权重，把原目标总销量分摊到 12 个 SKU；同一 SPU 下的
  SKU 销量不同，但重复导入结果稳定，且分摊前后的 SPU 总销量不变。
