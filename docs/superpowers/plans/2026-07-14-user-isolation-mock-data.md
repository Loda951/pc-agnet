# User Isolation Mock Data Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an idempotent PostgreSQL seed script that creates five login-capable test users, five traceable orders and two chat conversations per user, then prints ownership summaries for user-isolation testing.

**Architecture:** Keep deterministic fixture construction separate from async SQLAlchemy persistence so IDs, statuses and output can be unit-tested without PostgreSQL. The persistence layer accepts an existing `AsyncSession`, upserts only namespaced records inside the caller's transaction, rejects ownership collisions, and lets the CLI own commit/rollback and exit behavior.

**Tech Stack:** Python 3.11, SQLAlchemy 2 async ORM, asyncpg, PostgreSQL, pytest, pytest-asyncio, Ruff.

## Global Constraints

- Use 4-space Python indentation, complete type annotations and a 100-character Ruff line limit.
- Create exactly 5 users named `test_user_001` through `test_user_005`.
- Use the public Demo password `isolation-demo-password`, but persist it with the existing PBKDF2 `PasswordHasher` format.
- Create exactly 5 orders and 2 chat conversations per user.
- Never insert `user_session`; real `/api/auth/login` calls create authentication sessions.
- Do not modify existing Alembic migrations or add an order schema migration.
- Do not delete or overwrite non-script-owned data.
- Keep all writes in one transaction and report success only after commit.
- Tests must not require a real LLM API key.

## File Structure

- Create `backend/scripts/seed_user_isolation.py`: fixture specifications, deterministic ID builders, ownership guards, SQLAlchemy upserts, transaction entry point and CLI summary.
- Create `backend/tests/test_seed_user_isolation.py`: always-running pure-function tests plus an optional PostgreSQL idempotency test using the existing transaction fixture.

---

### Task 1: Deterministic fixture specifications

**Files:**
- Create: `backend/scripts/seed_user_isolation.py`
- Test: `backend/tests/test_seed_user_isolation.py`

**Interfaces:**
- Consumes: `datetime`, `Decimal` and the fixed constants defined in this task.
- Produces: `UserSeedSpec`, `OrderSeedSpec`, `ConversationSeedSpec`, `SeedSummary`, `build_user_specs()`, `build_order_specs()`, `build_conversation_specs()`, `assert_expected_owner()` and `format_summary()`.

- [ ] **Step 1: Write failing pure-function tests**

Create `backend/tests/test_seed_user_isolation.py` with these tests:
...
- [ ] **Step 2: Run the test file and confirm the import fails**

Run:

```bash
cd backend
.venv/bin/pytest tests/test_seed_user_isolation.py -q
```

Expected: collection fails with `ModuleNotFoundError: No module named 'scripts.seed_user_isolation'`.

- [ ] **Step 3: Implement deterministic specifications and formatting**

Create `backend/scripts/seed_user_isolation.py` with:
...
ORDER_STATUS_LABELS = {1: "待付款", 2: "待发货", 3: "已发货", 4: "已完成", 5: "已关闭"}
...
def assert_expected_owner(
    record_type: str, record_id: int, actual_owner_id: int, expected_owner_id: int
) -> None:
...
- [ ] **Step 5: Commit deterministic fixture layer**

```bash
git add backend/scripts/seed_user_isolation.py backend/tests/test_seed_user_isolation.py
git commit -m "feat: define user isolation mock data"
```

---

### Task 2: Transactional PostgreSQL upserts

**Files:**
- Modify: `backend/scripts/seed_user_isolation.py`
- Modify: `backend/tests/test_seed_user_isolation.py`

**Interfaces:**
- Consumes: Task 1 specifications, `AsyncSession`, project ORM models, `PasswordHasher`.
- Produces: `seed_user_isolation(session: AsyncSession, anchor: datetime | None = None) -> SeedSummary`, which flushes but does not commit; `seed_in_transaction() -> SeedSummary`, which owns the transaction.

- [ ] **Step 1: Add a failing PostgreSQL idempotency test**
...
from app.models import AppUser, Conversation, OrderInfo
...
async def test_seed_user_isolation_persists_owned_data_idempotently(
    ...
    async with db_session_factory() as session:
        ...
        order_count = await session.scalar(
            select(func.count(OrderInfo.id)).where(OrderInfo.id.between(991000000101, 991000000505))
        )
        conversation_count = await session.scalar(
            select(func.count(Conversation.id)).where(
                Conversation.id.between(992000000101, 992000000502)
            )
        )
    ...
    assert user_count == 5
    assert order_count == 25
    assert conversation_count == 10
```
...
```bash
cd backend
.venv/bin/pytest tests/test_seed_user_isolation.py::test_seed_user_isolation_persists_owned_data_idempotently -q
```

Expected when PostgreSQL is available: import fails because `seed_user_isolation` is undefined. If PostgreSQL is unavailable, test collection still proves the missing import before fixture setup.

- [ ] **Step 3: Implement focused persistence helpers**

Add imports for `select`, `or_`, `AsyncSession`, `AsyncSessionLocal`, all required ORM models, and `PasswordHasher`. Add the stable fallback catalog markers and implement these helpers:
        ...
        user = (
            await session.execute(
                select(AppUser).where(
                    AppUser.login_identifier == spec.login_identifier
                )
            )
        ).scalar_one_or_none()
        if user is None:
...
async def _upsert_credential(
    session: AsyncSession, user: AppUser, spec: UserSeedSpec
) -> None:
    credentials = (
        (
            await session.execute(
    ...
    if conflicting is not None:
        assert_expected_owner(
            "credential", conflicting.id, conflicting.user_id, user.id
        )
    credential = next(
        (credential for credential in credentials if credential.user_id == user.id),
        None,
...
async def _upsert_order_item(
    session: AsyncSession, order: OrderInfo, spec: OrderSeedSpec, sku: Sku
) -> None:
...
async def _upsert_logistics(
    session: AsyncSession, order: OrderInfo, user_ordinal: int, spec: OrderSeedSpec
) -> None:
...
async def _upsert_conversation(
    ...
    if conversation is None:
        ...
        assert_expected_owner(
            "conversation", conversation.id, conversation.user_id, user.id
        )
...
async def _upsert_message(
    session: AsyncSession, conversation: Conversation, spec: MessageSeedSpec
) -> None:
    ...
    if message is None:
        ...
        assert_expected_owner(
            "message", message.id, message.conversation_id, conversation.id
        )
...
- [ ] **Step 4: Implement the orchestration functions**

Add:

```python
async def seed_user_isolation(
    session: AsyncSession, anchor: datetime | None = None
) -> SeedSummary:
    ...
    for user_spec in user_specs:
        ...
        for conversation_spec in build_conversation_specs(
            user_spec.ordinal, normalized_anchor
        ):
...
`_upsert_users()` must upsert all five users and credentials before `_get_or_create_sku()` or any order operation, preserving the required dependency order.

- [ ] **Step 5: Run all seed-script tests**
...
Expected: `7 passed` when PostgreSQL is available, or `6 passed, 1 skipped` otherwise.

- [ ] **Step 6: Commit transactional persistence**

```bash
git add backend/scripts/seed_user_isolation.py backend/tests/test_seed_user_isolation.py
git commit -m "feat: seed isolated user orders and conversations"
```

---

### Task 3: CLI failure handling and final verification
...
**Interfaces:**
- Consumes: `seed_in_transaction()` and `format_summary()`.
- Produces: `main() -> int` and executable `python -m scripts.seed_user_isolation` behavior.

- [ ] **Step 1: Add failing CLI behavior tests**

Append tests using `monkeypatch` and `capsys`:
...
def test_main_prints_summary_only_after_success(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        "scripts.seed_user_isolation.seed_in_transaction", successful_seed
    )
...
def test_main_reports_failure_without_success_summary(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr("scripts.seed_user_isolation.seed_in_transaction", failed_seed)
...
- [ ] **Step 2: Run the two CLI tests and confirm `main` is missing**

Run:

```bash
cd backend
.venv/bin/pytest tests/test_seed_user_isolation.py -k "main_" -q
```

Expected: fails because `main` is not defined.

- [ ] **Step 3: Implement CLI error handling**
...
cd backend
.venv/bin/pytest tests/test_seed_user_isolation.py -q
.venv/bin/ruff check scripts/seed_user_isolation.py tests/test_seed_user_isolation.py
```

Expected: tests pass (with only the existing PostgreSQL availability skip if needed) and Ruff reports `All checks passed!`.

- [ ] **Step 5: Run the script twice against PostgreSQL when available**
...
.venv/bin/python -m scripts.seed_user_isolation
.venv/bin/python -m scripts.seed_user_isolation
```

Expected: both executions return 0 and each prints 5 user lines, 25 order lines and 10 conversation lines with the same IDs and owners.

- [ ] **Step 6: Run repository-level backend verification**
...
Expected: all available tests pass and Ruff reports `All checks passed!`.

- [ ] **Step 7: Commit the completed CLI and verification tests**

```bash
git add backend/scripts/seed_user_isolation.py backend/tests/test_seed_user_isolation.py
git commit -m "test: verify user isolation mock seed"
```
